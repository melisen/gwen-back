...
const filterByCategory = async (category) => {
  try {
    ...
    logger.log(
      "error",
      "/api/productos/:category -  GET  - error al mostrar catálogo por categoría"
    );
  }
};
...
const findProductAndUpdate = async (obj) => {
  const modifProd = await DAOproductos.findProdUpdate(obj);
  return modifProd;
};
